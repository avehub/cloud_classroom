...
#站点域名(不包括http)
SITE_DOMAIN=study.gaodekuai.com
...
#统一创建www用户（兼容所有Linux发行版）
if ! id -u www &>/dev/null; then
  useradd -r -s /sbin/nologin www 2>/dev/null || true
fi

#设置ctc目录属主
chown -R www:www ${ctc_dir}

#修改mysql数据目录属主
chown -R ${MYSQL_UID}:${MYSQL_UID} ${base_dir}/mysql/log ${base_dir}/mysql/log
...
#使用www用户运行，避免权限问题
docker exec -i --user www:www ctc-php bash <<'EOF'
