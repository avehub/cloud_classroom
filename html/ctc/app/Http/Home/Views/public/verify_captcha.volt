        ...
        layui.use(['jquery', 'form', 'layer', 'helper'], function () {
            ...
            var startCountdown = function () {
                ...
                if (verify.type === 'phone') {
                    $emit = parent.layui.$('#cv-phone-emit-btn');
                } else if (verify.type === 'email') {
                    $emit = parent.layui.$('#cv-email-emit-btn');
                } else {
                    $emit = parent.layui.$('#cv-emit-btn');
                }
                if (!$emit.length || $emit.attr('data-counting') === '1') return;
                $emit.attr('data-counting', '1').attr('disabled', 'disabled').addClass('layui-btn-disabled');
                var seconds = 60;
                var originText = $emit.text();
                $emit.text('重新发送(' + seconds + 's)');
                var timer = setInterval(function () {
                    seconds--;
                    if (seconds <= 0) {
                        clearInterval(timer);
                        if ($emit.attr('data-counting') === '1') {
                            $emit.text(originText).removeAttr('disabled').removeClass('layui-btn-disabled').removeAttr('data-counting');
                        }
                    } else {
                        $emit.text('重新发送(' + seconds + 's)');
                    }
